...
def test_sync_with_store_enable_document(ingestion_service):
    ...
    simba_doc = SimbaDoc(id="doc1", documents=[chunk1, chunk2], metadata=metadata)

    # Configure mocks
    ingestion_service.database.get_all_documents.return_value = [simba_doc]
    ingestion_service.vector_store.chunk_in_store.side_effect = (
        lambda x: True
    )  # Chunks exist
...
def test_sync_with_store_disable_document(ingestion_service):
    ...
    simba_doc = SimbaDoc(id="doc1", documents=[chunk1], metadata=metadata)
...
def test_sync_with_store_error_handling(ingestion_service):
    ...
    # Setup
    ingestion_service.database.get_all_documents.side_effect = Exception(
        "Database error"
    )
